    ...
    try:
        with storage.session_scope() as session:
            ...
            stored_location = (
                {
                    ...
                    "lat": (
                        float(client.latitude) if client.latitude else None
                    ),
                    "lng": (
                        float(client.longitude) if client.longitude else None
                    ),
                }
                if client.address
                else None
            ...
            return render_template(
                "delivery.html", stored_location=stored_location
            )
...
@login_required
def save_location() -> Dict[str, Any]:
